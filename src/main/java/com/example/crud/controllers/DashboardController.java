...
public class DashboardController {
    ...
    public String dashboard(@AuthenticationPrincipal Usuario usuarioLogado, Model model) {
        ...
        boolean temImobiliarias = imovelRepository.countByUsuario(usuarioLogado) > 0;
        boolean temClientes = clienteService.contarTodos(usuarioLogado) > 0;
        boolean temImoveis = imovelRepository.countByUsuario(usuarioLogado) > 0;
        boolean temNegociacoes = negociacaoRepository.countByUsuario(usuarioLogado) > 0;

        boolean mostrarWizard = !(temImobiliarias && temClientes && temImoveis && temNegociacoes);
        model.addAttribute("mostrarWizard", mostrarWizard);
